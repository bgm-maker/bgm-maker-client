...
위의 페이지는 음원의 파형을 시각화하고, 특정 구간을 선택해 잘라내거나  
Fade In/Out, 볼륨 조절, Compressor(강조할 곳을 더 강조해주는 이펙트)등 **편집 기능을 제공하는 페이지**이다.

처음에는 하나의 음원 소스를 기반으로 파형 시각화와 이펙트 조절을 동시에 처리하려 했으나,  
이를 모두 지원하는 단일 라이브러리를 찾을 수 없었다.

그래서 **파형을 그려주는 작업**은 -> **WaveSurfer.js** 라이브러리  
**음원의 이펙트 처리**는 -> **Tone.js** 라이브러리를 사용하였다.  
즉, **하나의 음원으로부터 WaveSurfer.js 와 Tone.js 각각 인스턴스를 생성해 병행 사용**하였다.  
...
이를 활용하여, 사용자가 음원을 잘라내면 WaveSurfer.js 로 잘라낸 뒤 Audio Context 를 추출해서  
